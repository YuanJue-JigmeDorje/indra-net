...
(function(){
  ...
  function showEntityCard(eid, etype) {
    ...
    if (typeof links !== 'undefined') {
      // Strip 《》 for fuzzy matching
      var eidBare = eid.replace(/[《》]/g, '');
      var rels = links.filter(function(l) {
        var s = lsrc(l), t = ltgt(l);
        // Exact match
        if (s === eid || t === eid) return true;
        // Fuzzy: bare name substring match (金刚橛 matches 《金刚橛》法部)
        if (eidBare.length >= 2) {
          if (s.indexOf(eidBare) >= 0 || t.indexOf(eidBare) >= 0) return true;
        }
        return false;
      });
      if (rels.length > 0) {
    ...
    var evtIndices = null;
    if (typeof allEvents !== 'undefined' && typeof entityEventIndex !== 'undefined') {
      evtIndices = entityEventIndex[eid];
      // Fuzzy: try bare name if exact not found
      if (!evtIndices && eidBare.length >= 2) {
        for (var key in entityEventIndex) {
          if (key.indexOf(eidBare) >= 0 || eidBare.indexOf(key.replace(/[《》]/g,'')) >= 0) {
            evtIndices = entityEventIndex[key];
            break;
          }
        }
      }
